...
pub async fn plugin_system_run(
    ...
    let output = match tokio::time::timeout(timeout, command.output()).await {
        ...
        Ok(Err(err)) => {
            ...
            let mut fallback_dirs = well_known_command_dirs();
            if let Some(staged) = plugin_sidecar_bin_dir(&app, &plugin_id) {
                fallback_dirs.push(staged);
            }
            let Some(path) = resolve_in_well_known_dirs(&bin, &fallback_dirs) else {
                return Err(format!("failed to run '{bin}': {err}"));
            };
...
/// binaries. `plugin_system_run` resolves allowlisted bare names against this
/// directory after `PATH` and the well-known installation directories.
fn plugin_sidecar_bin_dir(app: &AppHandle, plugin_id: &str) -> Option<PathBuf> {
...
/// this command re-verifies transit (`path in manifest.files`, `sha256` cited
/// per target) before any bytes are used, so `plugin_system_run` can execute
/// the staged binary by its allowlisted bare name.
#[tauri::command]
pub async fn plugin_sidecar_stage(
    ...
    // The staged file name must be an allowlisted bare executable name, so a
    // plugin can never swap an allowlisted resolution onto another binary.
    ...
    let url = generate_url(
        &["/api/v1/plugins", &plugin_id, "client", &asset],
        &[],
    )
