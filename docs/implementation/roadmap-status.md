...
| #19 Dropworks SDK & Multiplayer    | 🟡     | Server REST contract (`/api/v1/dropworks/session`, `/achievement`, `/leaderboard`, `/presence`) with token-derived users, leaderboard best-score retention and presence wired to the local presence service ✅; bindings: TypeScript reference client, Rust crate, tested C ABI (`libdropworks`, C smoke test), C# P/Invoke wrapper (buildable smoke test) and Godot 4 GDScript (`HTTPClient`, headless test) covering session/achievement/leaderboard/presence ✅; The server-side ICE config provider (STUN + ephemeral coturn TURN credentials at GET /api/v1/client/ice) is in place ✅; the client WebRTC peer-connection stack / hole-punching remains ⛔.                                                                                                                                                              |
