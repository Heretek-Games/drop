...
4. ### Create your launch script
   ...
   # export DROP_CERTIFICATE_KEY=<32-byte hex or base64 key> # seal certificate private keys at rest
...
Drop's certificate authority and per-client private keys live in the database.
Set `DROP_CERTIFICATE_KEY` to a 32-byte key (64 hex characters or base64) to
seal them with AES-256-GCM, so a database dump or backup does not expose a
usable signing key. Keys sealed with a key can only be read while that key is
configured, so store it with your other deployment secrets.

When `DROP_CERTIFICATE_KEY` is unset, Drop logs a warning at startup and stores
certificate private keys unencrypted to preserve compatibility with existing
installs.
