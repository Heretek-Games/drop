...
If you've built the Drop server from source, or checked out the logs in the container, you may have noticed an application called `torrential`. `torrential` is a download server that's embedded into Drop, and serves the chunks required for the Depot API, without actually assembling them ahead of time.
:::
